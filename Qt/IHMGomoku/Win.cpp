...
{
	if (pos % 19 < 16 && tab[pos - 1] != tab[pos + 2] &&
		tab[pos] == tab[pos + 1] &&	tab[pos + 1] != '-' &&
		tab[pos] != tab[pos - 1] && tab[pos] != tab[pos + 2])
	{
		return 1;
	}
	if (pos % 19 > 2 && tab[pos + 1] != tab[pos - 2] &&
		tab[pos] == tab[pos - 1] &&	tab[pos - 1] != '-' &&
		tab[pos] != tab[pos + 1] && tab[pos] != tab[pos - 2])
	{
		return 1;
	}
	if (pos < 303 && tab[pos - 19] != tab[pos + 19 * 2] &&
		tab[pos] == tab[pos + 19] && tab[pos + 19] != '-' &&
		tab[pos] != tab[pos - 19] && tab[pos] != tab[pos + 19 * 2])
	{
		return 1;
	}
	if (pos > 56 && tab[pos + 19] != tab[pos - 19 * 2] &&
		tab[pos] == tab[pos - 19] && tab[pos - 19] != '-' &&
		tab[pos] != tab[pos + 19] && tab[pos] != tab[pos - 19 * 2])
	{
		return 1;
	...
	if (pos < 303 && pos % 19 < 16 &&
		...
		tab[pos] == tab[pos + 1 + 19] && tab[pos + 1 + 19] != '-' &&
